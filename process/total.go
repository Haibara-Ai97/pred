...
func total(ctx context.Context, db *gorm.DB, point *sql.ThresholdPoints, rmq *mq.RabbitMQService,
	pointInfo *sql.PointProgress, modelParams models.ModelParams, dataSource datasource.DataSource,
	evalDay time.Time, staticChan chan<- StaticStats, isEmergency bool) (ErrorStats, error) {
	// error处理
	...
	prepareDay := pointInfo.PredictTime // 已处理进度
	...
	if prepareDay.IsZero() || prepareDay.Before(endTime1) || isEmergency {
		if point.CalFlag == 0 {
			return makeError("PointExcluded", fmt.Errorf("point: %s is excluded", point.PointName))
		}

		var DBOrigin []*sql.YSBase
		...
		if isEmergency {
			DBOrigin, err = dataSource.GetLastOriginData(ctx, point.Tablename, point.PointName, startTime, endTime2)
		} else {
			DBOrigin, err = dataSource.GetOriginData(ctx, point.Tablename, point.PointName, startTime, endTime2)
		}
		if err != nil {
			return makeError("OriginSQLFailed", err)
		}
		...
		if err = rmq.SendTotalRequest(ctx, req); err != nil {
			log.Printf("failed to send TotalRequest: %v", err)
			return makeError("SendTotalRequest", err)
		}

		resp, err := rmq.ReceiveTotalResponse(ctx, req.RequestID)
		if err != nil {
			log.Printf("failed to receive TotalResponse: %v", err)
		...
		err = db.Transaction(func(tx *gorm.DB) error {
			err = sql.ToPreprocess(tx, point, fullOrigin, fullPrepare, idMap)
			if err != nil {
				return err
			}
			return nil
		...
		if isEmergency {
			err = db.Transaction(func(tx *gorm.DB) error {
				err = sql.UpdateEmergencyFlag(tx, point, DBOrigin[0].ID)
				if err != nil {
					return err
				}
				return nil
		...
		log.Printf("Successfully gross & predict point: %s", point.PointName)
	}
